...
"""
An all_objects_of_class function has been added to previous versions. 
It returns a sorted list of objects from a given table (class). 
To avoid repetition the UID_to_object code from the value_to_ORM_object 
function has been separated out into a helper function. 
...
def all_objects_of_class(table_name, column):
    """
    Takes a database table name 
    Returns a list of all values in given column name
    """
    ...
    cursor = cnx.cursor()
    cursor.execute("SELECT UID FROM " + table_name + " ORDER BY " + column + ";")
    # An ordered list of singleton tuples: 
    column_values = cursor.fetchall()
    
    objects = []
    # Use class ID_to_object dict to store all ORM_objects 
    # to avoid duplication
    for _tuple in column_values:
