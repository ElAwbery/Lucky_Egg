...
The ORM_object code assumes that user defined types (classes) have identical table names in the database. This means that all object attribute values can be stored to and loaded from the database with generic code:
 ...
 - After I normalized the database (see below) I realized that cross references between database tables using foreign keys could lead to multiple loads of one object. To avoid this I wrote the an ID_to_object class dictionary that guarantees an object is loaded only once per interpreter session.
...
I normalized the database to reflect the new class structure:
 - Built two new database tables with names identical to the __pokemon_species__ and __pokemon_family__ classes. The __ORM_class__ code uses class names to access database tables. 
