...
"""
The all_objects_of_class function in previous version is updated to return 
a list of objects from a given table (class), optionally filtered by column 
name and value.  
    
The database_ORM handles all database operations. It is the interface 
between the application code and the database. We assume our user defined 
types (classes) have identical table names in the database so that all
...
The value_to_ORM_object helper function fetches objects from the class 
dictionary and calls the ORM_object class to instantiate new objects.
        ...
        for column_name in self.columns:
            ...
            if column_name in foreign_keys and attribute_value != None:
                ...
                # dictionary:
                    ...
                    _class = get_class_from_name(referenced_table)
...
def get_class_from_name(name):
    """
    name is a string, name of class or subclass
    returns the class or the subclass
    """
    
    assert type(name) == str
        
    for subclass in ORM_object.__subclasses__():

        if subclass.__name__ == name:
            return subclass
    
    # Can't find specified class
    raise TypeError

def UID_to_object(class_name, UID):
    """
    Given a UID and a class name, return the unique object of that class with 
    that UID.
    """
    if (class_name, UID) not in ORM_object.ID_to_object:
        _class = get_class_from_name(class_name)
...
def value_to_ORM_object(class_name, attribute_name, value):
    ...
    try:
        ...
        return "The request was for" + value + "not a Pokemon Species"
     
    UID = cursor.fetchone()[0]
    ORM_object = UID_to_object(class_name, UID)
...
# Updated from previous versions to take optional column and value arguments
# so that objects of class returned can be filtered by column name and value
def all_objects_of_class(table_name, column = None, value = None):
    """
    Takes a database table name and optional column, value
    Returns a list of all objects that have that value in given column
    or else all objects in the table if the column/value is None
    """
    # By ORM contract table_name and column must be identical to an ORM_object
    # subclass and one of its attributes
    
    cnx = mysql.connector.connect(user='root', password='root',
                                  host='127.0.0.1', port="8889",
                                  database = database_name)
    # Construct SQL
    SQL = "SELECT UID FROM " + table_name
    
    if value:
        SQL += " WHERE " + column + " = " + str(value)

    # No need for a prepared cursor because this deals only with application
    # code, no browser input
    cursor = cnx.cursor()
    cursor.execute(SQL)
    # A list of singleton tuples: 
    uid_tuples = cursor.fetchall()
    
    cursor.close()
    cnx.close()
    
    objects = []
    # Use class ID_to_object dict to store all ORM_objects 
    # to avoid duplication
    for _tuple in uid_tuples:
        _object = UID_to_object(table_name, int(_tuple[0]))
